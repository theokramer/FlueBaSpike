              ...
              return GestureDetector(
                ...
                child: ListTile(
                  title: Column(children: [
                    Row(
                      spacing: 10.0, 
                      children: [Text(contactList[index].name), Text(roles[contactList[index].rolle])],
                    ),
                    Row(
                      spacing: 5.0,
                      children: [Text(contactList[index].mail), Text(contactList[index].text)]
                    )
                  ]), 
                  horizontalTitleGap: 5.0, 
                  tileColor: (index % 2 == 0) ?const Color.fromARGB(255, 177, 177, 177) : const Color.fromARGB(255, 141, 141, 141)),
              );
            }, )
          ),
      ));
      ...
      showDialog(builder: (context) {
          return StatefulBuilder(
            builder: (context, setState) {
              return AlertDialog(title: Text("Add Participant"), content: Column(children: [
                  MyFormField(myController: nameController, text: "Name",),
                  DropdownButton<String>(items: widget.roles.map((role) {
                    return DropdownMenuItem<String>(
                      value: role,
                      child: Text(role),
                    );
                  }).toList(), value: widget.roles[selectedRole], onChanged: (value) {
                setState(() {
